...
app.get("/api/tweets", (req, res) => {
  (0, _nodeFetch.default)("https://foyzulhassan.github.io/files/favs.json").then(res => res.json()).then(data => {
...
app.get("/api/tweet/:id", (req, res) => {
  (0, _nodeFetch.default)("https://foyzulhassan.github.io/files/favs.json").then(res => res.json()).then(data => {
    ...
    let selectedTweet = data.filter(tweet => tweet.id === tweetId).map(tweet => {
      return {
        created_at: tweet.created_at,
        text: tweet.text,
...
app.get("/api/user/:username", (req, res) => {
  (0, _nodeFetch.default)("https://foyzulhassan.github.io/files/favs.json").then(res => res.json()).then(data => {
    ...
    let userProfile = data.filter(tweet => tweet.user.screen_name === username).map(tweet => {
      return {
        screen_name: tweet.user.screen_name,
